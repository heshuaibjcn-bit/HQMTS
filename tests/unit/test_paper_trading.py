...
class TestPaperTradingEngine:
    ...
    @pytest.mark.asyncio
    async def test_submit_buy_signal(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        signal = _make_signal()
        order = await engine.submit_signal(signal, reference_price=Decimal("10.00"))
    ...
    @pytest.mark.asyncio
    async def test_submit_buy_insufficient_cash(self):
    ...
    @pytest.mark.asyncio
    async def test_submit_sell_signal_no_position(self):
    ...
    @pytest.mark.asyncio
    async def test_submit_sell_with_position(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        buy_signal = _make_signal()
        buy_order = await engine.submit_signal(buy_signal, Decimal("10.00"))
        await engine.fill_order(buy_order, Decimal("10.00"))
        engine.end_of_day_reset()

        sell_signal = _make_signal(signal_type=SignalType.CLOSE_LONG)
        sell_order = await engine.submit_signal(sell_signal, Decimal("10.50"))
        assert sell_order.side == "sell"
        assert sell_order.status == PaperOrderStatus.PENDING
        assert sell_order.quantity > 0

    @pytest.mark.asyncio
    async def test_fill_buy_order(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        signal = _make_signal()
        order = await engine.submit_signal(signal, Decimal("10.00"))
        filled = await engine.fill_order(order, Decimal("10.00"))
    ...
    @pytest.mark.asyncio
    async def test_fill_buy_updates_cash(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        signal = _make_signal()
        order = await engine.submit_signal(signal, Decimal("10.00"))
        cash_before = engine.state.cash
        await engine.fill_order(order, Decimal("10.00"))

        assert engine.state.cash < cash_before

    @pytest.mark.asyncio
    async def test_fill_buy_creates_position(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        signal = _make_signal()
        order = await engine.submit_signal(signal, Decimal("10.00"))
        await engine.fill_order(order, Decimal("10.00"))
    ...
    @pytest.mark.asyncio
    async def test_fill_sell_order(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        buy_signal = _make_signal()
        buy_order = await engine.submit_signal(buy_signal, Decimal("10.00"))
        await engine.fill_order(buy_order, Decimal("10.00"))
        ...
        sell_signal = _make_signal(signal_type=SignalType.CLOSE_LONG)
        sell_order = await engine.submit_signal(sell_signal, Decimal("11.00"))
        filled = await engine.fill_order(sell_order, Decimal("11.00"))

        assert filled.status == PaperOrderStatus.FILLED
        assert filled.stamp_tax > Decimal("0")  # Stamp tax on sell

    @pytest.mark.asyncio
    async def test_fill_sell_increases_cash(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        buy_signal = _make_signal()
        buy_order = await engine.submit_signal(buy_signal, Decimal("10.00"))
        await engine.fill_order(buy_order, Decimal("10.00"))
        ...
        sell_signal = _make_signal(signal_type=SignalType.CLOSE_LONG)
        sell_order = await engine.submit_signal(sell_signal, Decimal("11.00"))
        await engine.fill_order(sell_order, Decimal("11.00"))

        assert engine.state.cash > cash_before

    @pytest.mark.asyncio
    async def test_fill_sell_removes_position_when_empty(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        buy_signal = _make_signal()
        buy_order = await engine.submit_signal(buy_signal, Decimal("10.00"))
        await engine.fill_order(buy_order, Decimal("10.00"))
    ...
    @pytest.mark.asyncio
    async def test_fill_rejected_if_not_pending(self):
        order = PaperOrder(
            order_id="o1", signal_id="s1", instrument_id="000001.SZ",
            side="buy", order_type="market", price=Decimal("10"), quantity=100,
    ...
    @pytest.mark.asyncio
    async def test_end_of_day_reset(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        signal = _make_signal()
        order = await engine.submit_signal(signal, Decimal("10.00"))
        await engine.fill_order(order, Decimal("10.00"))
    ...
    @pytest.mark.asyncio
    async def test_end_of_day_snapshot(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        signal = _make_signal()
        order = await engine.submit_signal(signal, Decimal("10.00"))
        await engine.fill_order(order, Decimal("10.00"))
    ...
    @pytest.mark.asyncio
    async def test_get_total_asset_with_prices(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        signal = _make_signal()
        order = await engine.submit_signal(signal, Decimal("10.00"))
        await engine.fill_order(order, Decimal("10.00"))
    ...
    @pytest.mark.asyncio
    async def test_get_total_asset_without_prices(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        signal = _make_signal()
        order = await engine.submit_signal(signal, Decimal("10.00"))
        await engine.fill_order(order, Decimal("10.00"))
    ...
    @pytest.mark.asyncio
    async def test_cost_price_averaging_on_second_buy(self):
        engine = PaperTradingEngine(session_id="s1", strategy_name="test", initial_cash=CASH)
        # First buy
        signal1 = _make_signal()
        order1 = await engine.submit_signal(signal1, Decimal("10.00"))
        await engine.fill_order(order1, Decimal("10.00"))
        engine.end_of_day_reset()

        # Second buy at different price
        signal2 = _make_signal()
        order2 = await engine.submit_signal(signal2, Decimal("12.00"))
        await engine.fill_order(order2, Decimal("12.00"))
