...
class TestAgentAuditService:
    def test_record_task_created(self):
        ...
        event = svc.record_task_created("task-1", "research_agent", "Analyze trend factors")
    ...
    def test_record_policy_evaluation(self):
        svc = AgentAuditService()
        event = svc.record_policy_evaluation("task-1", "research_agent", "denied", ["no_approval"])
    ...
    def test_record_proposal_submitted(self):
        svc = AgentAuditService()
        event = svc.record_proposal_submitted("prop-1", "task-1", "data_agent", "data_query")
    ...
    def test_record_approval_decision_approved(self):
        svc = AgentAuditService()
        event = svc.record_approval_decision("prop-1", "human_operator", "approved", "Looks good")
    ...
    def test_record_controlled_execution(self):
        svc = AgentAuditService()
        event = svc.record_controlled_execution("exec-1", "prop-1", "system", "completed")
    ...
    def test_record_unauthorized_attempt(self):
        svc = AgentAuditService()
        event = svc.record_unauthorized_attempt("data_agent", "write_order", "No write permission")
    ...
    def test_record_task_state_change(self):
        svc = AgentAuditService()
        event = svc.record_task_state_change("task-1", "research_agent", "created", "planning")
    ...
    def test_events_are_immutable(self):
        svc = AgentAuditService()
        event = svc.record_task_created("task-1", "research_agent", "test")
    ...
    def test_environment_propagated(self):
        ...
        event = svc.record_task_created("task-1", "agent", "test")
        assert event.environment == Environment.LIVE

    def test_correlation_id_propagated(self):
        svc = AgentAuditService()
        event = svc.record_task_created("task-1", "agent", "test", correlation_id="corr-123")
        assert event.correlation_id == "corr-123"

    def test_unique_audit_event_ids(self):
        svc = AgentAuditService()
        e1 = svc.record_task_created("task-1", "agent", "a")
        e2 = svc.record_task_created("task-2", "agent", "b")
